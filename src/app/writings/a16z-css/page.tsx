...
import { A16Z_ARTICLES } from "@/lib/data";
import { cn } from "@/lib/utils";
import Link from "next/link";

export default function A16zCss() {
  // Extract unique tags and prepend "All"
  const allTags = Array.from(new Set(A16Z_ARTICLES.flatMap(a => a.tags)));
  const tags = ["All", ...allTags];
  
  // Extract unique years for the left heading
  const allYears = Array.from(new Set(A16Z_ARTICLES.map(a => a.year))).sort((a, b) => Number(b) - Number(a));
  ...
  // Filter articles
  const filteredArticles = A16Z_ARTICLES.filter(a => {
  ...
  return (
    <div className="pt-[80px] md:pt-[96px] pb-[40px] min-h-screen flex flex-col px-6 md:px-10 w-full max-w-[1100px] mx-auto animate-fade-in bg-black text-white">
      ...
      <div className="mb-10 flex flex-col gap-4 items-start">
        <Link href="/writings" className="text-white/40 hover:text-white transition-colors text-sm font-mono lowercase flex items-center gap-2">
          <span>←</span> back to writings
        </Link>
        <h1 className="text-[2rem] md:text-[2.5rem] font-medium tracking-tight leading-tight">
          a16z Crypto Startup School
        </h1>
        <p className="text-white/60 text-sm md:text-base max-w-[600px]">
          Notes and curriculum highlights from the a16z Crypto Startup School (CSS).
        </p>
      </div>
